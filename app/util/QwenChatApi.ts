...
export class QwenChatApi{
    ...
    chatStream: { [Symbol.asyncIterator](): { next(): (Promise<unknown>); return(): Promise<{ done: boolean; }>; }; } | undefined;
    constructor({model='qwen-turbo',api_key=QwenChatApi.API_KEY}={}){
        this.model = model
        QwenChatApi.API_KEY = api_key
    }
    iteratorToStream(iterator: any) {
    ...
    async *makeIterator() {
        try {
            ...
            for await
                (const message of this.chatStream) {
    ...
    streamChat(messages:Message[]=[], history:Message[]= []) {
     ...
     * @param history
     */
    stream(messages:Message[]=[], history:Message[]= []) {
